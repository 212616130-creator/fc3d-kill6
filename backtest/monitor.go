...
// Kill6Entry kill6_history.json 的一条记录
type Kill6Entry struct {
	Issue string  `json:"issue"`
	Date  string  `json:"date"`
	Pct   float64 `json:"pct"`
}

// RecordKill6 把近 N 期 6 杀全中率追加到历史文件；同期号覆盖；只留最近 400 条。
func RecordKill6(path string, pct float64, issue, date string) ([]Kill6Entry, error) {
	hist := []Kill6Entry{}
	...
	if len(hist) > 0 && hist[len(hist)-1].Issue == issue {
		...
		hist = append(hist, Kill6Entry{Issue: issue, Date: date, Pct: round1(pct)})
	}
...
// CheckUpgradeTrigger 检测两个升级触发条件，返回 (是否触发, 原因, 单月下滑pp)。
func CheckUpgradeTrigger(curPct float64, hist []Kill6Entry) (bool, []string, float64) {
	reasons := []string{}
	if curPct < TrigBelowPct {
		reasons = append(reasons, formatPct(curPct)+" 跌破 "+formatPct(TrigBelowPct)+" 触发阈值")
	}
	drop := 0.0
	if len(hist) >= 2 {
